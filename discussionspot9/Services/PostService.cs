...
namespace discussionspot9.Services
{
    public class PostService : IPostService
    {
        ...
        /// <summary>
        /// Retrieves detailed information for a single post based on its category slug and post slug.
        /// </summary>
        /// <param name="categorySlug">The slug of the category the post belongs to.</param>
        /// <param name="postSlug">The slug of the post.</param>
        /// <returns>A PostDetailViewModel containing the post details, or null if not found.</returns>
        public async Task<PostDetailViewModel?> GetPostDetailsByCategoryAsync(string categorySlug, string postSlug)
        {
            try
            {
                var post = await _context.Posts
                    .Include(p => p.Community)
                        .ThenInclude(c => c.Category) // Include Category through Community
                    ...
                    .Include(p => p.Awards)
                        .ThenInclude(pa => pa.AwardedByUser)
                    .Where(p => p.Slug == postSlug && p.Community.Category.Slug == categorySlug)
                    .FirstOrDefaultAsync();
                ...
                var viewModel = new PostDetailViewModel
                {
                    ...
                    UserId = post.UserId,
                    AuthorUserName = post.UserProfile?.DisplayName ?? "Deleted User",
                    AuthorAvatarUrl = post.UserProfile?.AvatarUrl,
                    CommunityId = post.CommunityId,
                    CommunityName = post.Community?.Name ?? "Unknown Community",
                    CommunitySlug = post.Community?.Slug ?? "unknown-community",
                    CommunityIconUrl = post.Community?.IconUrl,
                    CategoryName = post.Community?.Category?.Name ?? "Unknown Category", // Get category name from community's category
                    CategorySlug = post.Community?.Category?.Slug ?? "unknown-category", // Get category slug from community's category
                    ...
                    Awards = post.Awards.Select(pa => new PostAwardViewModel
                    {
                        ...
                        AwardedByUserId = pa.AwardedByUserId,                
                        // With this corrected line:
                        AwardedByUserName = pa.AwardedByUser?.UserName ?? "Anonymous",                      
                        AwardedAt = pa.AwardedAt,
                        Message = pa.Message,
                        IsAnonymous = pa.IsAnonymous
                    }).ToList(),
                    Poll = post.HasPoll && post.PollConfiguration != null ? new PollViewModel
                    {
                        AllowMultipleChoices = post.PollConfiguration.AllowMultipleChoices,
                        ShowResultsBeforeVoting = post.PollConfiguration.ShowResultsBeforeVoting,
                        ShowResultsBeforeEnd = post.PollConfiguration.ShowResultsBeforeEnd,
                        AllowAddingOptions = post.PollConfiguration.AllowAddingOptions,
                        MinOptions = post.PollConfiguration.MinOptions,
                        MaxOptions = post.PollConfiguration.MaxOptions,
            ...
            {
                _logger.LogError(ex, "Error getting post details by category slug: {CategorySlug}, post slug: {PostSlug}", categorySlug, postSlug);
                return null;
            }
        }
