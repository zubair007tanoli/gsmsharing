class SignalRManager {
    ...
    async handleDelegatedCommentClick(event) {
        ...
        if (voteButton) {
            ...
            if (!isNaN(commentId) && !isNaN(voteType)) {
                this.setLoadingState(voteButton, true);
                try {
                    await this.voteComment(commentId, voteType);
                } finally {
                    this.setLoadingState(voteButton, false);
                }
            }
            return;
        }
        ...
        if (submitReplyButton) {
            ...
            if (content && this.pagePostId) {
                ...
                if (!isNaN(parentId)) {
                    this.setLoadingState(submitReplyButton, true);
                    try {
                        ...
                        this.setLoadingState(submitReplyButton, false);
                    }
                }
            } else if (!content) {
    ...
    async handlePostVoteButtonClick(event) {
    ...
    async handlePollVote(event) {
        ...
        if (!pollContainer || pollContainer.classList.contains('voted')) return;
        ...
        if (!isNaN(postId) && !isNaN(optionId)) {
            const voteButton = optionContainer.querySelector('.poll-option-vote-btn');
            if (voteButton) this.setLoadingState(voteButton, true);

            try {
                await this.castPollVote(postId, optionId);
            } finally {
                if (voteButton) this.setLoadingState(voteButton, false);
            }
        }
    }
    ...
    showNotification(message, type = 'info') {
        ...
        const bgColors = {
            'error': '#dc3545', 'success': '#198754', 'info': '#0d6efd', 'warning': '#ffc107'
        };
            ...
            submitButton.addEventListener('click', async (e) => {
                ...
                if (contentElement) {
                    ...
                    if (content && signalRManager.pagePostId) {
                        signalRManager.setLoadingState(submitButton, true);
                        try {
                            await signalRManager.sendComment(signalRManager.pagePostId, content);
                            contentElement.value = '';
                        } catch (err) {
                            console.error("Error sending main comment:", err);
                            signalRManager.showNotification('Failed to post comment.', 'error');
                        } finally {
                            signalRManager.setLoadingState(submitButton, false);
                        }
                    } else if (!content) {
        ...
        if (mainCommentContent) {
            mainCommentContent.addEventListener('keydown', async (e) => {
                if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    document.getElementById('submitMainComment')?.click();
                }
            });
        }
