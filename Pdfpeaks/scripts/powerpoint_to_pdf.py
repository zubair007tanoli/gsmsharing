#!/usr/bin/env python3
"""
PowerPoint to PDF conversion using LibreOffice headless (free, no commercial libraries).
...
def main(input_pptx, output_pdf):
    try:
        if not os.path.exists(input_pptx):
            print(f"ERROR: Input file not found: {input_pptx}", file=sys.stderr)
            sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("ERROR: Usage: powerpoint_to_pdf.py <input_pptx> <output_pdf>", file=sys.stderr)
        sys.exit(1)
